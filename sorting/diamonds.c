...
int main()
{
    int numOps, key, count;
    scanf("%d", &numOps);
    char query[200];
    char op;

    BSTNode *root = CreateNode(0, 0);
    // BSTNode *root;
    ...
    for (int i = 0; i < numOps + 1; i++)
    {
        ...
        sscanf(query, "%c %d %d", &op, &key, &count);
        sscanf(query, " ");
        if (op == 'C') {
            val = Count(root, key);
        ...
        else if (op == 'I')
        {
            ...
            Delete(root, key, count);
        else if (op == 'R') {
            val = RangeCount(root, key, count);
...
void Delete(BSTNode *root, Key key, Data count) {
    ...
    {  
        ...
        if (cur->key > key)
        {
            ...
            cur = cur->lc;
            if (cur->lChildren < 0) cur->lChildren = 0;
        ...
        else
        {
            ...
            if (cur->rChildren < 0) cur->rChildren = 0;
            cur = cur->rc;
        }
